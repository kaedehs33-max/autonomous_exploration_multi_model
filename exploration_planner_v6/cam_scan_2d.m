function omap_local_new = cam_scan_2d(omap_local, omap_lidar, robot_pose, max_range, angles)
...
for i = 1:length(angles)
    ...
    [~, mid_points] = raycast(omap_lidar,start_pt,end_pt);
    if isempty(mid_points)
        continue;
    end
    for j = 1:size(mid_points, 1)-1
        uv = mid_points(j, :);
        xy = grid2world(omap_lidar, uv);
        occ = getOccupancy(omap_lidar, xy);
        d = norm(xy - start_pt);
        if occ >= 0.65
            if d > min_range
