classdef RHNBV_RRT_1 < handle
    properties
        ...
        best_branch_id % id of node in rrt of best branch
        cache
        best_path
        ig_cam
        ig_cam_threshold
    ...
    methods
        ...
        function finish_flag = run(obj)
            ...
            while find_ig == false && iter <= obj.max_iter
                ...
                [best_ig_cam, ~] = max(obj.ig_cam);
                if (best_ig_raw > obj.finish_threshold || best_ig_cam > obj.finish_threshold) &&...
                        nbv_id ~= 1 % high predicted information gain
                    find_ig = true;
...
function restart_tree(obj, new_omap_lidar, new_omap_cam)
            ...
            for i = 2:size(obj.tree, 1)
                ...
                %new_ig = new_ig_raw * exp(-obj.lambda * obj.distance(i)) + 2*new_ig_cam * exp(-obj.lambda * obj.distance(i));
                ...
                while add_new_node == 0
                    ...
                    for i = 1:3
                        ...
                        if occ < 0.2
                            ...
                            %new_ig = new_ig_raw * exp(-obj.lambda * new_distance) + 2*new_ig_cam * exp(-obj.lambda * new_distance);
                            new_ig = new_ig_raw * exp(-obj.lambda * obj.distance(i));
                            obj.ig = [obj.ig; new_ig];
                            break;
                        end
                ...
                for i = 1:length(inrange_id)
                    ...
                    if obj.distance(id) + d < obj.distance(new_node_id)
                        ...
                        if occ < 0.2
                            ...
                            %obj.ig(new_node_id) = (obj.ig_raw(new_node_id)+2*obj.ig_cam(new_node_id)) * exp(-obj.lambda * obj.distance(new_node_id));
                ...
                for i = 1:length(inrange_id)
                    ...
                    if obj.distance(id) > obj.distance(new_node_id) + d
                        ...
                        if occ < 0.2
                            ...
                            %obj.ig(id) = (obj.ig_raw(id)+2*obj.ig_cam(id)) * exp(-obj.lambda * obj.distance(id));
