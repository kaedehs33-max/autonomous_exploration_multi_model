...
robot_pose = [1, 11, 0]+[pad_meters, pad_meters, 0];

%% RHNBV_RRT

% scan
omap_local_lidar = lidar_scan_2d(omap_local_lidar, omap_gt, robot_pose, max_range_lidar, angles);
omap_local_cam = cam_scan_2d(omap_local_cam, omap_local_lidar, robot_pose, min_range_cam, max_range_cam, angles);
...
aux.min_range_cam = min_range_cam;
aux.max_range_cam = max_range_cam;
aux.angles = angles;
aux.step_size = 1;
aux.reconnect_range = 1.5 * aux.step_size;
aux.default_iter = 20;
aux.max_iter = 140;
aux.finish_threshold = 0.5;
aux.tree = aux.start; % Tree initialization
aux.lambda = 0.4;
aux.alpha = 0;    % importance of cam over lidar
...
while finish_flag == false
    ...
    disp(['ig_lidar' max(num2str(RHNBV_RRT_planner.ig_raw_lidar))]);
    disp(['ig_cam' max(num2str(RHNBV_RRT_planner.ig_raw_cam))]);
    pause(0.1);
end
