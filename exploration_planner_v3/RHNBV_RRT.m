classdef RHNBV_RRT < handle
    properties
        ...
        omap_lidar    % occupancy map of lidar
        omap_cam      % occupancy map of cam
        pad_meters 
        max_range_lidar     % Lidar parameter 
        min_range_cam
        max_range_cam       % cam parameter
    ...
    methods
        ...
        function obj = RHNBV_RRT(aux)
            ...
            obj.min_range_cam = aux.min_range_cam;
...
%%
        function restart_tree(obj, new_omap_lidar, new_omap_cam)
            ...
            for i = 2:size(obj.tree, 1)
                new_node = obj.tree(i, :);
                [new_ig_raw, new_ig_raw_lidar, new_ig_raw_cam] = information_gain(obj.omap_lidar, obj.omap_cam, ...
                    [new_node, 0], obj.max_range_lidar, obj.min_range_cam, ...
                    obj.max_range_cam, ...
                    obj.angles, obj.alpha);
                obj.ig_raw = [obj.ig_raw; new_ig_raw];
                obj.ig_raw_lidar = [obj.ig_raw_lidar; new_ig_raw_lidar];
                obj.ig_raw_cam = [obj.ig_raw_cam; new_ig_raw_cam];
            ...
            for iter = 1:iter_num
                ...
                while add_new_node == 0
                    ...
                    for i = 1:3
                        ...
                        occ = getOccupancy(obj.omap_lidar, grid2world(obj.omap_lidar, [mid_points; end_points]));
                        if occ < 0.2
                            add_new_node = 1;
                            ...
                            [new_ig_raw, new_ig_raw_lidar, new_ig_raw_cam] = information_gain(obj.omap_lidar, obj.omap_cam, ...
                                [new_node, 0], obj.max_range_lidar, ...
                                obj.min_range_cam, obj.max_range_cam, ...
                                obj.angles, obj.alpha);
                            obj.ig_raw = [obj.ig_raw; new_ig_raw];
                            obj.ig_raw = [obj.ig_raw; new_ig_raw];
                            obj.ig_raw_lidar = [obj.ig_raw_lidar; new_ig_raw_lidar];
